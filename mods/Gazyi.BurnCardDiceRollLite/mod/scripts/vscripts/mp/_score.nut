...
/*
	...
	Titan - ?
	Scores in TF2:
	Earning Titan - 420 s ( 1% every 4.2 seconds )
	Defualt kill bonuses (Earn+Bonus):
	Grunt - 4% (2+2)
	Spectre - 4% (2+2)
	Stalker - 4% (2+2)
	Reaper - 10% (0+10)
	...
	Titan - 15% (0+15)
*/
float function GetPlayerXPMultipler( entity player, ScoreEvent event )
{
	if ( PlayerHasServerFlag( player, SFLAG_FAST_BUILD2 ) )
		return 2.0
	
	if ( event.name == "KillGrunt" && PlayerHasServerFlag( player, SFLAG_HUNTER_GRUNT ) )
		return 2.0
	
	if ( ( event.name == "KillSpectre" || event.name == "KillStalker" || event.name == "KillSuperSpectre" || event.name == "LeechSpectre" ) && PlayerHasServerFlag( player, SFLAG_HUNTER_SPECTRE ) )
		return 2.0

	if ( event.name == "KillPilot" && PlayerHasServerFlag( player, SFLAG_HUNTER_PILOT ) )
		return 2.0
