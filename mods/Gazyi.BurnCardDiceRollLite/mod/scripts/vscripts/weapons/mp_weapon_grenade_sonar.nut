...
struct
{
	...
	table< entity, array<int> > entitySonarEMPHandles
	table< entity, array<int> > entitySonarJammerHandles
		...
		foreach ( weapon in offhandWeapons )
		{
			...
			if ( weapon.GetWeaponClassName() == "mp_weapon_grenade_sonar" )
			{
				...
				if ( weapon.HasMod( "burn_mod_emp_grenade" ) )
					trigger.SetScriptName( "SonarJammer" )
...
void function OnSonarTriggerEnter( entity trigger, entity ent )
{
	...
	{
		if ( !(ent in file.entitySonarEMPHandles) )
			file.entitySonarEMPHandles[ent] <- []

		if ( !(ent in file.entitySonarJammerHandles) )
			file.entitySonarJammerHandles[ent] <- []

		int statusEffectEMPHandle = StatusEffect_AddEndless( ent, eStatusEffect.emp, 0.4 )
		file.entitySonarEMPHandles[ent].append( statusEffectEMPHandle )

		int statusEffectJammerHandle = StatusEffect_AddEndless( ent, eStatusEffect.minimap_jammed, 1.0 )
		file.entitySonarJammerHandles[ent].append( statusEffectJammerHandle )
	}
}
...
void function OnSonarTriggerLeave( entity trigger, entity ent )
{
	...
	if ( trigger.GetScriptName() == "SonarJammer" )
	{
		if ( file.entitySonarEMPHandles[ent].len() )
		{
			...
			file.entitySonarEMPHandles[ent].fastremovebyvalue( statusEffectHandle )
		}
		if ( file.entitySonarJammerHandles[ent].len() )
		{
			int statusEffectHandle = file.entitySonarJammerHandles[ent][0]
			StatusEffect_Stop( ent, statusEffectHandle )
			file.entitySonarJammerHandles[ent].fastremovebyvalue( statusEffectHandle )
		}
	}
}
